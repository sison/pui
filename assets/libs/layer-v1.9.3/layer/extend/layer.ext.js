...
layer.CKEditorPrompt = function(options, yes){
    options = options || {};
    if(typeof options === 'function') yes = options;
    var prompt, content = options.formType == 2 ? '<textarea id="ckeditor" class="layui-layer-input">'+ (options.value||'') +'</textarea>' : function(){
        return '<input type="'+ (options.formType == 1 ? 'password' : 'text') +'" class="layui-layer-input" value="'+ (options.value||'') +'">';
    }();
    return layer.open($.extend({
        btn: ['&#x786E;&#x5B9A;','&#x53D6;&#x6D88;'],
...
layer.MultipleOptionPrompt = function(options, yes){
    options = options || {};
    if(typeof options === 'function') yes = options;
    var prompt, content = options.formType == 2 ? '<span class="multipleoption-msg">输入值:(每行一个,最长200字符)</span><textarea class="layui-layer-input-multipleoptionprompt">'+ (options.value||'') +'</textarea>' : function(){
        return '<input type="'+ (options.formType == 1 ? 'password' : 'text') +'" class="layui-layer-input" value="'+ (options.value||'') +'">';
    }();
    return layer.open($.extend({
        btn: ['&#x786E;&#x5B9A;','&#x53D6;&#x6D88;'],
        ...
        area: ['640px', '480px'],
        success: function(layero){
            prompt = layero.find('.layui-layer-input-multipleoptionprompt');
            prompt.focus();
        }, yes: function(index){
            var value = prompt.val();
            if(value === ''){
